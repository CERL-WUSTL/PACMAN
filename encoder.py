...
class MixtureofExpertsEncoder(Encoder):
    ...
    def gram_schmidt_orthogonalization(self, x: TensorType) -> TensorType:
        num_experts, batch_size, feature_dim = x.shape

        # Initialize the orthogonalized tensor with the same shape as x
        ...
        for i in range(num_experts):
            ...
            for j in range(i):
                # Clone to ensure no in-place modification
                q_j = q[j].clone()
                proj = torch.sum(q_j * v, dim=1, keepdim=True) * q_j
                v = v - proj

            # Normalize the vector
            q[i] = F.normalize(v, p=2, dim=1)
    ...
    def forward(self, mtobs: MTObs, detach: bool = False, temperature: float = 0.5):
        ...
        if self.MOORE:
            encoding = self.gram_schmidt_orthogonalization(encoding)
